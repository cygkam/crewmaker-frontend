...
class AddOpinion extends Component {
  ...
  componentWillMount() {
    ...
    console.log(this.props.opinionData);
    if (this.props.opinionData !== null) {
      this.setState({
        eventPlaceOpinionID: this.props.aboutEventPlace,
        title: {
  ...
  handleSubmit() {
    const newOpinionRequest = {
      eventPlaceAbout: this.props.aboutEventPlace,
    ...
    eventPlaceOpinionService
      .sendEventPlaceOpinion(newOpinionRequest)
      .then((response) => {
        notification.success({
          message: "Nowa opinia!",
          description: "Dziękujemy! Twoja opinia została dodana pomyślnie.",
        });
        this.props.handler();
      })
      .catch((error) => {
        notification.error({
          ...
          description:
            error.message ||
            "Przeprasza, coś poszło nie tak. Spróbuj jeszcze raz!",
        });
      });
  }
  ...
  render() {
    ...
    if (
      this.state.title.validateStatus === "success" ||
      this.state.title.validateStatus == null
    ) {
      titleInput = (
        <Form.Input
          required
    ...
    if (
      this.state.message.validateStatus === "success" ||
      this.state.message.validateStatus == null
    ) {
      messageInput = (
        <Form.Input
          required
            ...
            <Segment width={5} textAlign="left" color="orange" padded>
              <Form>
                ...
                <Rating
                  size="huge"
                  icon="heart"
                  defaultRating={this.state.grade}
                  maxRating={5}
